...
#
# Extended to parse array's / Jens Rosemeier 2012
...
#echo line = "$line"
        ...
        then
            ISARRAY=1
            # remove the Array definition brackets [ ]
            ARRAY="${VAL%%\]}"
            ARRAY="${ARRAY##\[}"
            ARRAY=( ${ARRAY[*]//,/ } )
            # remove single/double quotes from array values
            for (( i=0; i<=$(( ${#ARRAY[*]} -1 )); i++ ))
            do
                arval="${ARRAY[$i]}"
                arval="${arval##\"}"
            ...
            # assign parsed array to original VAL
            VAL=( "${ARRAY[@]}" )
        fi
