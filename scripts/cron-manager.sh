...
# Ref: http://stackoverflow.com/questions/17598135/checking-for-duplicate-cron-jobs

cd $(dirname "$0")

test "$2" = --remove && mode=remove || mode=add

cron_unique_label="# cron-manager ID$PWD|$1#"

crontab="$1".crontab
TSTAMPNOW=`date +%s`
crontab_bak="`basename ${crontab}`.$TSTAMPNOW"
...
if type crontab >/dev/null 2>/dev/null; then
    if test $mode = add; then
        ...
        echo '-----------------------------------------------'
        echo 'To remove previously added crontab entry, run:'
        echo
        echo "$0 $1 --remove"
        echo
